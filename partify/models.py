...
class PlayQueueEntry(Base):
    ...
    user_priority = Column(Integer, default=lambda: db_session.query(func.max(PlayQueueEntry.user_priority)).first()[0] + 1)
    playback_priority = Column(BigInteger, unique=True, default=lambda: db_session.query(func.max(PlayQueueEntry.playback_priority)).first()[0] + 1)
